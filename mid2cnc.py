...
def main(argv):
	...
	if printer_mode == '1motor':
		...
		machine_limit_x = 100.93 # 
		machine_limit_y = 100.93 # 
		machine_limit_z = 10 # 
		machine_limit_e = 100.93 # 
		tempo_multiplier = float(argv[7])
		test_motors = argv[8];
	else:
		tone_multiplier = float(argv[7])
		num_axes = 4
		machine_ppi = [float(argv[3])/tone_multiplier,float(argv[4])/tone_multiplier,float(argv[5])/tone_multiplier,float(argv[6])/tone_multiplier]  # Higher numbers make the pitch lower and the song slower
		machine_limit_x = 100.93 # 
		machine_limit_y = 100.93 # 
		machine_limit_z = 10 # 
		machine_limit_e = 100.93 # 
		tempo_multiplier = float(argv[8])
		test_motors = argv[9];
	...
	for track in midi.tracks:
		for event in track.events:
			if event.type == midiparser.meta.SetTempo:
				tempo=event.detail.tempo*tempo_multiplier
	...
	if suppress_comments == 0:
		...
		FILE.write ("( e = " + str(machine_limit_e) + " )\n")
	...
	for note in noteEventList:
		...
		if last_time < note[0]:
			...
			freq_xyz=[0,0,0,0]
			feed_xyz=[0,0,0,0]
			distance_xyz=[0,0,0,0]

			if test_motors == 'yes':
				for i in range(0, num_axes):
					if len(active_notes.values()) > 0:
						nownote=sorted(active_notes.values(), reverse=True)[0]
						freq_xyz[i] = pow(2.0, (nownote-69.0)/12.0)*440.0 # convert note numbers to frequency for each axis in Hz
						feed_xyz[i] = freq_xyz[i] * 60.0 / machine_ppi[i]	# feedrate in IPM for each axis individually
			...
			print "Chord: [%.3f, %.3f, %.3f, %.3f] for %d deltas" % (freq_xyz[0], freq_xyz[1], freq_xyz[2], freq_xyz[3], (note[0] - last_time))
			...
			e = e + (distance_xyz[3] * e_dir)
			if e > (machine_limit_e-machine_safety):
				e_dir = -1
			if e < machine_safety:
				e_dir = 1
			...
			distance_sum = 0.0;
			for i in range(0, num_axes):
				distance_sum += distance_xyz[i];
			
			if distance_sum > 0: # handle 'rests' in addition to notes. How standard is this pause gcode, anyway?
				feed_sum = 0.0;
				for i in range(0, num_axes):
